  ...
  public static function stripe_callback( WP_REST_Request $request ) {
    ...
    try {
      $event = \Stripe\Webhook::constructEvent(
        $payload, $sig_header, payment_page_setting_get( 'stripe_' . $mode . '_webhook_secret' )
      );
    ...
    foreach( $paymentPageElements as $paymentPageElement ) {
      if( !isset( $paymentPageElement[ 'submit_actions' ] )
          || empty( $paymentPageElement[ 'submit_actions' ] ) )
        continue;

      if( in_array( "http_request", $paymentPageElement[ 'submit_actions' ] )
          && isset( $paymentPageElement[ 'http_request_url' ] )
          && !empty( $paymentPageElement[ 'http_request_url' ] )
          && !empty( $paymentPageElement[ 'http_request_url' ][ 'url' ] ) ) {
        $request_data = [
        ...
        $log->content[ 'request_response' ] = wp_remote_post(
          $paymentPageElement[ 'http_request_url' ][ 'url' ],
          [
            'body' => $request_data
          ]
        );
    ...
    // Create Read of Stripe Request Data and var_dump.
    // Do HTTP Request for Post_id if present.
